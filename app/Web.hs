  ...
  div_ [class_ "bg-slate-600 dark:bg-slate-700 w-full mx-auto pb-2 pt-2"] $ do
    div_ [class_ "px-8"] $ do
      div_ [class_ "max-w-screen-xl w-full mx-auto px-4 flex place-content-between text-md pt-1 pb-2 text-lg"] $ do 
        span_ [id_ "currentSong", class_ "text-orange-200 mr-4"] $ toHtmlRaw ("&nbsp;" :: T.Text)
        ...
        tbody_ $ mapM_ (\song ->
                          tr_ [class_ "flex place-content-between px-2 my-0"] $ do
                           ...
                           td_ [class_ "flex gap-x-2"] $ do
                             a_ [href_ $ "/browse?path=" <> (T.pack $ FP.takeDirectory $ MPD.toString $  MPD.sgFilePath song), class_ "pl-0 pr-4 w-1 hover:dark:text-blue-600 dark:text-blue-300 cursor-pointer my-auto"] $ i_ [class_ "size-4 stroke-3", data_ "feather" "search"] ""
